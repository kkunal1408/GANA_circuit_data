...
class create_combination:
    ...
    def __init__(self, module1, module2, module3):
        ...
        for mod in [self.module1, self.module2, self.module3]:
            ...
            self.rename_bias(mod)
            logging.debug(f"modified bias pins {mod.class_type} {mod.name} {mod.pins}")

    def rename_bias(self, mod):
        for i, pin in enumerate(mod.pins):
            if 'bias' in pin.lower():
                mod.pins[i] = pin.lower()+'_'+mod.class_type
    ...
    def merge_pins(self):
        ...
        if self.module2.class_type == 'mixer':
            ...
            mixer_lo_in = [pin for pin in self.module2.pins if 'vlo' in pin.lower()]
        if self.module3.class_type == 'lo':
            lo_out = [pin for pin in self.module2.pins if 'vlo' in pin.lower()]
